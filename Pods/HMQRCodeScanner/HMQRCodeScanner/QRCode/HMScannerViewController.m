...
- (void)clickAlbumButton {
    ...
    if (![UIImagePickerController isSourceTypeAvailable:UIImagePickerControllerSourceTypePhotoLibrary]) {
        if ([[[NSUserDefaults standardUserDefaults]objectForKey:@"appLanguage"] isEqualToString:@"zh-Hans"]){
            ...
            tipLabel.text = @"Unable to access album";
        }
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary<NSString *,id> *)info {
    ...
    [HMScanner scaneImage:image completion:^(NSArray *values) {
        ...
        if (values.count > 0) {
            ...
            if ([[[NSUserDefaults standardUserDefaults]objectForKey:@"appLanguage"] isEqualToString:@"zh-Hans"]){
                tipLabel.text = @"没有识别到二维码";
            }else{
                tipLabel.text = @"The qr code is not recognized";
            }
...
- (void)prepareOtherControls {
    ...
    if ([[[NSUserDefaults standardUserDefaults]objectForKey:@"appLanguage"] isEqualToString:@"zh-Hans"]){
        ...
        tipLabel.text = @"The qr code is placed in the box to automatically scan.";
    }
...
- (void)prepareNavigationBar {
    ...
    if ([[[NSUserDefaults standardUserDefaults]objectForKey:@"appLanguage"] isEqualToString:@"zh-Hans"]){
        ...
        // 3> 左右按钮
        self.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:@"Close" style:UIBarButtonItemStylePlain target:self action:@selector(clickCloseButton)];
        self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:@"Album" style:UIBarButtonItemStylePlain target:self action:@selector(clickAlbumButton)];
    }
}
